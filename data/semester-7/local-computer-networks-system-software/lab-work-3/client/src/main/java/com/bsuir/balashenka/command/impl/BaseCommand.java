...
abstract class BaseCommand implements Command {
    ...
    public final void validateRequiredTokens() throws WrongCommandFormatException {
        ...
        int numOfRequiredTokens = (int) availableCommandTokens.getTokens().stream().filter(CommandToken::isRequired).count();

        if (tokens.size() > numOfRequiredTokens) {
            throw new WrongCommandFormatException("This command should have only " + numOfRequiredTokens + " tokens.");
        }
