...
def main():
    ...
    for group in groups_communicators:
        if group != MPI.COMM_NULL:
            ...
            receive_array = np.zeros(MATRIX_DIM, dtype='i')

            handle = MPI.File.Open(group, 'mpi/matrix.bin', amode)

            handle.Read_at_all(group.Get_rank() * receive_array.nbytes, receive_array)
